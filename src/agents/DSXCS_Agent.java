...
public class DSXCS_Agent extends Base_XCS_Agent {
    ...
    public void calculateNextMove(long gaTimestep) throws Exception {
        ...
        if (historicActionSet.size() > Configuration.getMaxStackSize()) {
            ...
            first.processReward(this.getClassifierSet(), max_prediction);
    ...
    public void collectExternalReward(BaseAgent other_agent, int start_index, int action_set_size, boolean reward, boolean is_event) throws Exception {
        ...
        switch (Configuration.getExternalRewardMode()) {
            ...
            case Configuration.REWARD_SIMPLE:
                collectReward(start_index, action_set_size, reward, classifierSet.checkDegreeOfRelationship(((DSXCS_Agent)other_agent).classifierSet), is_event);
                break;
            case Configuration.REWARD_COMPLEX:
                //collectReward(start_index, action_set_size, reward, classifierSet.checkComplexDegreeOfRelationship(((DSXCS_Agent)other_agent).classifierSet), is_event);
                break;
            case Configuration.REWARD_NEW:
                collectReward(start_index, action_set_size, reward, classifierSet.checkDegreeOfRelationshipNew(((DSXCS_Agent)other_agent).classifierSet), is_event);
                break;
     ...
     private void collectReward(int start_index, int action_set_size, boolean reward, double factor, boolean is_event) throws Exception {
        ...
        for(int i = 0; i < action_set_size; i++) {
            if(is_event) {
                ...
                if(reward && factor == 1.0) {
                    double max_prediction = 0.0;
                    if(Configuration.isUseMaxPrediction()) {
                        max_prediction = historicActionSet.get(start_index - i + 1).getBestValue();
                    }
                    historicActionSet.get(start_index - i).rewardPrematurely(max_prediction);
                }
            } 
            historicActionSet.get(start_index - i).addReward(corrected_reward, factor);
        }
    }
// Den maxPrediction reward auch weitergeben... Verknüpfung der Felder!
    /**
    ...
    public void resetBeforeNewProblem() throws Exception {
        ...
        while(!historicActionSet.isEmpty()) {
            ...
            last.processReward(this.getClassifierSet(), max_prediction);
            last.destroy();
        }
